...
import { ListCheck, LogIn, LogOut } from "lucide-react";
  ...
  return (
    <aside className="w-[300px] bg-[#0D1B3E] text-white h-screen fixed left-0 top-0 flex flex-col py-8">
      ...
      <div className="mt-auto  ">
       { user ? <button onClick={handdleLogout} className="flex cursor-pointer items-center gap-3 py-4 px-10  text-[#8CA3CD] hover:text-white hover:bg-[#1A2B52] transition-all w-full text-left">
          <LogOut size={18} /> Logout
        </button>  : <Link href='/login' className="flex cursor-pointer items-center gap-3 py-4 px-10  text-[#8CA3CD] hover:text-white hover:bg-[#1A2B52] transition-all w-full text-left">
          <LogIn size={18} /> Login
        </Link>}
      </div>
    </aside>
